...
// (2026.08 V3/V4→V4/V5 재갱신 — 운영자의 구버전 승인이 만료되고 같은 날 새 버전으로 재승인된 것을
...
const PARTIAL_OVERRIDE = {
  ...
  // 단순 포함(includes) 비교로도, naverLookup()의 접두/접미 비교로도 못 잡는다(2026.09, 운영자 리포트 —
...
const NAVER_DENY = new Set([
  ...
  // (2026.09, 운영자 리포트 — 실제로는 486%). 여울동 단지는 이번 네이버 크롤링에 아예 없어서(수집 누락으로
  // 추정) 세대수는 K-apt 자동 매칭(940세대, matchKapt의 "이름 길이가 가까운 후보 우선" 로직으로 이미
  // 정상적으로 여울동 쪽을 채택함)으로 폴백되고, 용적률만 표시가 안 되는 상태가 됨 — 틀린 값보다는 낫다.
  "HS-동탄구|동탄역롯데캐슬",
  // 2026.09 — 전국 실거래 단지명 스캔에서 "동 불일치 + 준공년도 5년 이상 차이"로 걸린 142건을 K-apt
  // 독립 매칭(matchKapt, 네이버와 전혀 다른 소스)과 한 번 더 대조해, 세대수까지 서로 어긋나는(오차 5세대
  // 초과) 51건만 추림(운영자 확인: "준공년도랑 세대수 다른 건 다른 단지로 보면 됨"). 동만 다르고
  // 세대수·연식이 사실상 같은 나머지 91건(예: 다성이즈빌 — 여러 법정동에 걸친 대단지로 추정)은
  // 정상 매칭일 가능성이 높아 그대로 둠.
  "11110|현대", "11170|한남동동원베네스트", "11170|신동아", "11200|중앙하이츠", "11200|현대",
  "11200|한남하이츠", "11230|한신", "11230|동아", "11260|현대", "11350|건영", "11350|동신",
  "11500|청솔", "11530|한신", "11560|삼성", "11560|경남아너스빌", "11560|건영", "11560|우성2",
  "11620|삼성", "11680|가람", "11710|롯데캐슬", "11740|파크프라자", "11740|우성", "11740|한솔",
  "26230|화신", "26230|로얄", "26260|삼성", "26380|자유", "26380|우림", "26470|현대",
  "26710|이지더원", "28200|벽산", "28245|뉴서울1", "28245|아주", "28245|하나", "28245|중앙",
  "41111|장안", "41115|현대", "41131|신세계쉐덴", "41150|동성", "41150|신도", "41173|현대",
  "41271|월드", "41271|푸른마을주공2", "41310|우림", "41360|대림", "41360|KCC스위첸",
  "41390|대우", "41390|삼호", "41450|동일하이빌", "HS-동탄구|e편한세상동탄파크아너스",
  "HS-만세구|송산",
]);
