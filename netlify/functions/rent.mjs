...
// (deposit=보증금, monthlyRent=월세금 — 이 함수는 월세를 걸러내고 monthlyRent가 0인
//  "순수 전세" 계약만 남긴다. dealAmount 필드 자체가 없어 deposit을 amt로 사용)
...
const RTMS = "https://apis.data.go.kr/1613000/RTMSDataSvcAptRent/getRTMSDataSvcAptRent";

// 행정구역 개편 지역: analyze.mjs/presale.mjs와 동일 설정 (화성시 2026.02 분구 / 부천시 2024.01 구 재설치 / 인천 2026.07 행정체제 개편)
const SPLIT_REGIONS = {
  "HS-": {
    ...
    dongs: {
      ...
      "병점구": ["병점동","진안동","반정동","기배동","기산동","화산동","안녕동","황계동","배양동","반월동","송산동","정남면"],
      "효행구": ["봉담읍","매송면","비봉면","기안동"],
  ...
  // 인천 2026.07.01 행정체제 개편: analyze.mjs와 동일 (fetchShardIncheon()에서 별도 처리)
  "IC-": {
    split: "202607",
    codes: { "제물포구": "28125", "영종구": "28155", "서해구": "28275", "검단구": "28290" },
    islandDongs: ["운북동", "중산동", "운남동", "운서동", "을왕동", "남북동", "덕교동", "무의동"],
    geomdanDongs: ["마전동", "불로동", "대곡동", "원당동", "당하동", "오류동", "왕길동", "백석동", "시천동", "금곡동"],
  },
};
...
function parseItems(xml, ymFallback) {
  ...
  while ((m = re.exec(xml))) {
    ...
    // 월세금이 0(원)인 계약만 "순수 전세"로 인정 — 월세가 붙은 계약(반전세 포함)은 보증금만으로는
    // 실질 임대료를 비교할 수 없어 전세 시세 분석에서 제외한다.
...
async function fetchStaticMonth(origin, lawd, ym) {
  try {
    const r = await fetch(`${origin}/data/rent/${encodeURIComponent(lawd)}/${ym}.json`);
    if (!r.ok) return null;
    const j = await r.json();
